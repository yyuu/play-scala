...
class SqlTests extends UnitTestCase with ShouldMatchersForJUnit {
  ...
  @Test def useSomeMagicSql{
    ...
    Task.findSingle("where id={id}").on("id" -> 1).result() should be (Some(new Task("some comment")))
    Task.find().result() should be (List(new Task("some comment")))
    Task.find().result() should be (List(new Task("some comment")))
    Task.findSingle("where id={id}").on("id" -> 3).result() should be (None)
  ...
  @Test def insertAnEntity(){
    ...
    User2.create(User2(TODO,"Paul","Address")).right.get should be (User2(Id(1),"Paul","Address"))
  }
  @Test def updateAnEntity(){
    ...
    play.db.DB.execute("""CREATE TABLE User2 
                       ...
                       Address char(60) NOT NULL,
                       Name char(60) NOT NULL) """)
    val newUser:User2 = User2.create(User2(TODO,"Paul","Address")).right.get 
  ...
  @Test def count(){
    ...
    play.db.DB.execute("""insert into User2 (Address, Name) Values('address','Paul')""")
    User2.count().result() should be (2)
  }
