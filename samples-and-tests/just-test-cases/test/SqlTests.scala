...
class SqlTests extends UnitTestCase with ShouldMatchersForJUnit {
  ...
  object BBB extends MagicParser2[String,String,BBB](Some("Task"))
  ...
  @Test def useSomeMagicSqlCompileTime{
    ...
    play.db.DB.execute("""insert into Task Values('1','some comment')""")
    play.db.DB.execute("""insert into Student Values('1','1')""")
    object TT2 extends MagicParser2[String,String,(String,String)](Some("Task")) {
 ...
 implicit def statementInOut[A](implicit c:ColumnTo[A], ts:ToStatement[A]):(ColumnTo[A],ToStatement[A]) = (c,ts)
  ...
  object AAA extends Magic2[Pk[Int],String,AAA](Some("Task"))
