...
We have chosen to create yet another blog engine. It’s not a very imaginative choice but it will allow us to explore most of the functionality needed by a modern web application. 
...
Becuase we will be initially doing a lot of testing and changes in the application model, it’s probably better to use an in-memory database so we always start with a fresh data set.
