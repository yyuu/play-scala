...
Let’s see what the **controllers.Application** controller looks like. Open the **helloworld/app/controllers.scala** source file:
...
You can see that controller objects extend the **play.mvc.Controller** class. This class provides all useful methods for controllers. Also we have imported the @play.mvc._@ package that contain everything needed to write controllers.
...
p(note). Note that we use GET as the form submission method, because the form submission does not have any side effect and is idempotent.
...
Now our **sayHello** can return 2 distinct values. A @Html@ value is returned in the case where there is a name provided by the form, and an @Action@ value is returned if there is an error. An @Action@ value is used to redirect to another existing action method. Here we specify that we want to redirect to the @index@ action.
...
The flash scope allows Play to keep messages during action redirection.
...
See how we implicitly take the flash parameter from the controller scope.
