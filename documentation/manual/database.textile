...
h2. Using plain old SQL requests
...
We believe that it is the best way to access your relational databases from Scala and this component will be encouraged an fully integrated with the rest of the Play Scala stack.

Please check the "complete manual":sql for more informations.
...
* JPA relies a lot on Java annotations. And there are a several semantic differences that make Java and Scala annotations not really compatibles.

But the most important point is: do you really need an Relationnal to Objects mapper when you have the power of a functional language? Probably not. JPA is a convenient way to abstract the Java's lack of power in data transformation, but it really feels wrong when you start to use it from Scala.
...
For example, here are the few step need to integrate "ScalaQuery":http://scalaquery.org/ with your Play application.

h3. 1. Add ScalaQuery to your dependencies.yml file

"ScalaQuery":http://scalaquery.org/download.html is available from the "Scala Tools repository":http://scala-tools.org/repo-releases/org/scalaquery/. So open your application **conf/depenencies.yml** file, and add the following informations:
...
Now let's write a simple action method that query all the **Members** registred in our database:
