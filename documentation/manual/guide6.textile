...
Where we have tags, we need a tag cloud. Let’s add a method to the @Tag@ object to generate the tag cloud. The Scala version is simplier than the Java/JPA version. We don't need to use the JPQL new/map tricks. We will use the Parser Combinator, a very powerful feature of play-scala.
...
We need to declare 2 more entities in the bootstrap. Add 2 new lines in the @yabe/app/boostrap.scala@ file, one line to read and create @Tag@ and one line to read and create @TagsForPosts@: 
...
You need to restart your application to force loading of the new initial data set. The Posts are now correctly tagged but we haven't updated the UI yet.  	

h2. <a name="ui">Updating the UI to display tags</a>

Let's see how we can now display our tags on the web page. First, we will update the @Post@ case class and add a new metho, @getTags@. This method returns the list of @Tags@ for the current @Post@:
...
This function selects all tag's name for a specific @Post@ and returns a @List[String]@. 

Time to update the @display.scala.html@ tag now. We will use a different technique here. Instead of specifying a new template parameter, we can also call directly helper's method on object.
Add a new span for Tags after the @&lt;div class="post-metadata"&gt;@ section :
...
The @post._1@ returns the current @Post@. We apply the @getTags@ function that returns a @List[String]@. Finally, we use the @mkString@ of <a href="ttp://www.scala-lang.org/api/current/index.html#scala.collection.immutable.List">scala.collection.immutable.List</a> to transform the List of String to one single String, where each token is separated by a quote.

In this section you added a new helper method and updated the display tag. We will now see how we could create a new @action@ in our controller to show all posts by tag.

h2. <a name="uiAction">Adding a new action to display all posts by tag name</a>

To complete this tutorial, we will now create a new action that will display all posts for a specific tag. First, we will create a new action in the @Application@ controller, then we will update the @display.scala.html@ tag:
...
Reload and select a post, then select a tag, it should displays a List of @Post@ in your web browser. This is a little bit easier to test than the Java version. 

To complete this tutorial we will define the @listTagged@. Create the @/yabe/app/views/Application/listTagged.scala.html@ template that will be used by the new listTagged action:
...
Developer Note : the @posts.length@  does not work in v0.9.1 of play-scala. You will need to upgrade to the latest version of play-scala.
