...
p(note). One known limitation of the Scala Model API is that the Scala compiler can not infer the return type of *save*, *delete* and *edit* methods. So if you want to save/edit/delete a Model inplace, by convention you should use a so-called *Inplace* method, ie  *saveInplace*, *deleteInplace* and *editInplace*. If, however, you need the return type, you will need to explicitly ask for it ie. *var user2:User = user.save()* or *var u2 = user.save().asInstanceOf[User]* 
