...
"The API":http://github.com/guillaumebort/play-scala/blob/master/src/play/db/jpa/QueryOn.scala is really similar to the java so, so for example to count the number of users, you can just call count on the User class (assuming you defined the appropriate companion object):
...
And you can turn secure it by mixing in the **Secured** trait as well:
