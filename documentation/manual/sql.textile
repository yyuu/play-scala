h1. SQL data access with Play Scala

The Scala module includes a brand new data access layer the use plain SQL to make your database request and provides several API to parse and transform the resulting dataset. 
...
It can feel strange to back to plain old SQL to access an SQL Database these days. In particular for Java developers that are accustomed to use high level Object Relational Mapper like Hibernate to completely hide this aspect. 

Now if we agree that these tools are almost required in Java, we think that they are not needed at all we you have the power of an higher level programming language like Scala, and in the contrary they will quickly become counter productive.
...
But we provide a simpler API for JDBC, using Scala you don't need to bother with exceptions, and transforming data is really easy with Scala; in fact it is the point of the Play Scala SQL access layer to provide several API to effectively transform JDBC data into other Scala structures.
...
If you try to abstract this point with another proprietary SQL like DSL you will have to deal with several 'dialect' dedicated for each vendor (like Hibernate ones), and limit yourself of using interesting features of a particular Database.
    
Sometimes we will provide you with prefilled SQL statements. But the idea is not to hide you the fact that we use SQL under the hood. Just save a bunch of characters to type for trivial queries, and you can always back to plain old SQL.
...
And there are no guarantees at all that this MetaModel is correct. Even is the compiler says that you code and your queries are ok, it can still miserably fail at runtime because of a mismatch in your actual database definition.

h3. Take Control of your SQL code

bq. Object Relationnal Mapper work well for trivial cases. But when you have to deal with complex schema or existing database, you will spend most of your time to fight with your ORM to make it generate the SQL queries you want.
...
Well, import @play.db.sql._@, and then simply use the @SQL@ object to create queries.

bc. import play.db.sql._ 
 
val result:Boolean = SQL("Select 1").execute()

The @execute()@ method returns a @Boolean@ value indicating the execution was succesful. 
...
Use the @as(…)@ method of the SQL statement to specify the parser you want to use. For example @scalar[Long]@ is a simple parser that know how to parse a single column row as @Long@:
...
As this query use a **join**, our parser will need to span several rows of the ResultSet to generate a single item. We will use the @spanM@ combinator to construct this parser:

bc. str("name") ~< spanM(by=str("code"), str("language"))

Now let's use this parser to create a function that give us all languages spoken in a country:
...
Finally, based on all these concepts, Play provides a **Magic** helper that will help you to write parsers. The idea is that if you define a @case class@ that match a database table, Play Scala will generate a parser for you.

Let's try by defining a first @Country@ case class that describe the **Country** table:

bc. case class Country(
    code:Id[String], name:String, population:Int, headOfState:Option[String]
)

Note that we are not required to specify every existing table column in the case class. Only a subset is ok.
...
Also, @Magic@ provides automatically a set of methods that can generate basic SQL queries:
