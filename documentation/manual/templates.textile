...
They're particularly designed to feel comfortable to those used to working with HTML, allowing Web designers to work with them.

They are compiled as standard Scala functions, following a simple naming convention: 

bq. If you create a *views/Application/index.scala.html* template file, Play will generate a @views.Application.html.index@ function.
...
The Scala template uses *'@'* as a single special character. Each time this character is encountered, it indicates the beginning of a Scala statement. It does not require you to explicitly close the code-block, and will infer it from your code:

bc. Hello @customer.name!
      ^^^^^^^^^^^^^^
        Scala code

Because the template engine will automatically detect the end of your code block by analysing your code, it only allows for simple statements. If you want to insert a multi-token statement, just make it more explicit using brackets:
...
You can use the Scala @for comprehension@, in a pretty standard way. Just note that the template compiler will just add a @yield@ keyword before your block:
...
You can create reusable code blocks (or sub template):
...
And let's use it from a template:
