...
p(note). The @create@ method performs an INSERT into the database and try to retrieve the primary key from the ResultSet statement. This is usefull for an auto_increment primary key field. However, if your application sets the primary key id value and not the database, you must use the @insert@ method.
